...
# 当写正则式字符串的时候，相对普遍的做法是使用原始字符串
# 比如 r'(\d+)/(\d+)/(\d+)' 。 这种字符串将不去解析反斜杠，
# 这在正则表达式中是很有用的。 如果不这样做的话，你必须使用两个反斜杠，
# 类似 '(\\d+)/(\\d+)/(\\d+)' 。

# 注意的是 match() 方法仅仅检查字符串的开始部分。
# 它的匹配结果有可能并不是你期望的那样。
...
print(datepat2.match('11/27/2012abcdef'))
print(datepat2.match('11/27/2012'))

# 最后，如果你仅仅是做一次简单的文本匹配/搜索操作的话，
# 可以略过编译部分，直接使用 re 模块级别的函数。比如：
print(re.findall(r'(\d+)/(\d+)/(\d+)', text))

# 但是需要注意的是，如果你打算做大量的匹配和搜索操作的话，
# 最好先编译正则表达式，然后再重复使用它。 模块级别的函数
# 会将最近编译过的模式缓存起来，因此并不会消耗太多的性能，
